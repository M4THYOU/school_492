...
label after_uni_apply:
    ...
    menu:
        ...
        "Ivey League (Cost 70,000 per year)" if ivey_acept:
...
label after_intern_apply:
    ...
    python:
        ...
        if(startup_apply):
            intern_opts.append(JobOptions.Tech_Startup)
        if(minimum_apply):
    ...
    "Congratulation [user] you are going to work at [user.job] for the summer!"
...
label after_job_apply:
    ...
    python:
        ...
        if(startup_apply):
            first_job_opts.append(JobOptions.Tech_Startup)
        if(minimum_apply):
            first_job_opts.append(JobOptions.Minimum_Wage)

        accepted = job_apply(user, first_job_opts)
        faang_acept = JobOptions.FAANG in accepted
        local_acept = JobOptions.Local_IT_Company in accepted
        startup_accept = JobOptions.Tech_Startup in accepted
...
label after_first_job:
    ...
    "Congratulations on your completing Year [job_loop] of your fulltime career! You are working at Since you have been working for [job_loop] years, you can make the decision of whether to change jobs or apply for a promotion!"
